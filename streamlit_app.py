...
/* Theme (slightly higher contrast) */
:root{
  --bg:#F6EFEF;        /* page background */
  --panel:#FBECEC;     /* left pink panel */
  --border:#E1C1C3;
  --ink:#1b1e28;       /* darker text */
  --pill:#EAF3FF;      /* lighter blue pill */
  --pillborder:#C7DBFF;
  --hover:#EEF4FF;     /* light hover */
  --selected:#FFE7EC;  /* selected row */
}
...
.pill{display:inline-block;padding:6px 14px;border-radius:999px;background:var(--pill);border:1px solid var(--pillborder);font-weight:800;}
.pill-gray{display:inline-block;padding:6px 14px;border-radius:999px;background:#EEF0F3;border:1px solid #D7DBE0;font-weight:800;}
...
.hr{height:2px;background:var(--border);margin:8px 0;}
...
/* Event-look boxes (Cardiac/Septic/Risk) */
...
.stMultiSelect [data-baseweb="tag"] span{ color:#0b3c91 !important; }

/* the popover menu */
div[data-baseweb="popover"]{ z-index: 9999 !important; } /* ensure on top of gauge */
...
div[data-baseweb="menu"] [role="option"][aria-selected="true"]{
  background:var(--selected) !important; color:#111 !important;
  box-shadow: inset 3px 0 0 #E15259;
}
</style>
...
def gauge_svg(score_0_to_10: float) -> str:
    """Return an SVG gauge with the needle pointing according to score (0..10)."""
...
with col_left:
    ...
    st.markdown('<div style="padding:6px 10px;"><span class="pill-gray" style="font-size:12px;">Premature</span></div>', unsafe_allow_html=True)
    ...
    # Race (interactive -> pill mirrors)
    race = st.selectbox("Race", ["White","Black","Asian","Native American","Pacific Islander","Other"], index=0)
    st.markdown(f'<div class="row"><span><b>Race</b></span><span class="pill">{race}</span></div>', unsafe_allow_html=True)
    st.markdown('<div class="hr"></div>', unsafe_allow_html=True)

    # Ethnicity
    ethnicity = st.selectbox("Ethnicity", ["German","Hispanic/Latino","Non-Hispanic","Other"], index=0)
    st.markdown(f'<div class="row"><span><b>Ethnicity</b></span><span class="pill">{ethnicity}</span></div>', unsafe_allow_html=True)
    st.markdown('<div class="hr"></div>', unsafe_allow_html=True)

    # DOB
    dob = st.date_input("Date of Birth", value=dt.date(2025,4,21))
    dob_str = dob.strftime("%-m/%-d/%Y") if hasattr(dob, "strftime") else str(dob)
    st.markdown(f'<div class="row"><span><b>Date of Birth</b></span><span class="pill">{dob_str}</span></div>', unsafe_allow_html=True)
    st.markdown('<div class="hr"></div>', unsafe_allow_html=True)

    # Age @ Surgery (days)
    age_days = st.number_input("Age at Surgery (days)", min_value=0, value=7, step=1)
    st.markdown(f'<div class="row"><span><b>Age at Surgery</b></span><span class="pill-gray">{age_days} Days</span></div>', unsafe_allow_html=True)

    # weight mini tiles (visual)
    st.markdown("""
...
with col_mid:
    ...
    # Risk Factors (own box; dropdown directly above gauge)
    ...
    with st.container(border=True):
        ...
        weights = {
            "Premature": 2.5, "Low birth weight": 2.0, "Co-morbidity": 1.5,
            "Genetic syndrome": 1.5, "Recent infection": 1.0, "Post-op bleed": 3.0
        }
        score = sum(weights.get(x, 0) for x in risk_selection)
...
with col_right:
    ...
    st.markdown(
        f'<div class="card"><div style="text-decoration:underline;font-weight:900;">Syndrome Present: {syn}</div>'
        f'Fetal Drug Exposure: {fd}<br>Abnormalities: {ab}</div>',
        unsafe_allow_html=True
    )
